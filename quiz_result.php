...
if(!isset($_GET['score'], $_GET['total'], $_GET['playlist_id']))
...
if(!$check->fetch()){
    ...
    $conn->prepare("
        INSERT INTO quiz_certificates
        (student_id, playlist_id, score, passed)
        VALUES (?,?,?,?)
    ")->execute([
        $user_id,
        $playlist_id,
...
$date = date("d / m / Y H:i");
$cert_id = "CERT-" . strtoupper(substr(md5(time() . $user_id), 0, 10));
?>
...
.certificate{
    width: 900px;
    min-height: 1060px; /* زيادة الارتفاع */
    background: #fff;
    border: 12px solid #0f172a;
    position: relative;
    text-align: center;
    padding: 50px 40px 220px; /* مساحة سفلية للفوتر */
...
<div class="certificate" id="printArea">
    ...
    <div class="course">
        <?= htmlspecialchars($playlist_name); ?>
    </div>
...
<div class="footer">
    ...
    <div class="sign">
        <img src="images/stamp.png" alt="Platform Seal">
        <p>Platform Seal</p>
    </div>

    <div class="sign">
        <img src="images/signature.png" alt="Instructor Signature">
        <p>Instructor</p>
    </div>
